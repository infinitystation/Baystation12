...
/datum/evacuation_controller/proc/call_evacuation(var/mob/user, var/_emergency_evac, var/forced, var/skip_announce, var/autotransfer)
	...
	if(emergency_evacuation)
		...
		if(!skip_announce)
			priority_announcement.Announce(replacetext_char(replacetext_char(GLOB.using_map.shuttle_called_message, "%dock_name%", "[GLOB.using_map.dock_name]"),  "%ETA%", "[round(get_eta()/60)] minute\s"))
...
/datum/evacuation_controller/proc/finish_preparing_evac()
	...
	if (emergency_evacuation)
		evac_waiting.Announce(replacetext_char(GLOB.using_map.emergency_shuttle_docked_message, "%ETD%", "[estimated_time] minute\s"), new_sound = sound('sound/effects/Evacuation.ogg', volume = 35))
	else
		priority_announcement.Announce(replacetext_char(replacetext_char(GLOB.using_map.shuttle_docked_message, "%dock_name%", "[GLOB.using_map.dock_name]"),  "%ETD%", "[estimated_time] minute\s"))
