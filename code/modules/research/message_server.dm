	...
	for (var/obj/machinery/requests_console/Console in allConsoles)
		if (ckey(Console.department) == ckey(recipient))
			...
			if(priority > 1)
				...
				Console.audible_message("[icon2html(Console, world)]<span class='warning'>\The [Console] announces: 'High priority message received from [sender]!'</span>", hearing_distance = 8)
				...
				if(!Console.silent)
					...
					Console.audible_message("[icon2html(Console, world)]<span class='notice'>\The [Console] announces: 'Message received from [sender].'</span>", hearing_distance = 5)
...
/obj/machinery/message_server/proc/send_to_department(var/department, var/message, var/tone)
	...
	for(var/obj/item/device/pda/P in PDAs)
		...
		if(J.department_flag & department)
			P.new_info(P.message_silent, tone ? tone : P.ttone, "[icon2html(P, world)] [message]")
			reached++
	return reached
