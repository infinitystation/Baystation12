...
/obj/machinery/message_server/proc/send_to_department(var/department, var/message, var/tone)
	...
	for(var/mob/living/carbon/human/H in GLOB.human_mob_list)
		if(!(H.z in GLOB.using_map.station_levels)) // INF, странная логика, но пусть будет ~bear1ake
			continue
		var/obj/item/modular_computer/device = locate() in H
		var/obj/item/device/radio/headset/hs = locate() in H // INF, Дополнительно привлечем рации
		if(!device && !hs) // INF, было if(!device || !(get_z(device) in GLOB.using_map.station_levels)) | Причина, почему логика выше странная
			continue
		...
		if(J.department_flag & department)
			if(device) // INF, проверка устройства
				...
				to_chat(H, SPAN_NOTICE("Your [hs.name] vibrates and alerts you to the fact that somebody is requesting your presence at your department."))
				reached++
