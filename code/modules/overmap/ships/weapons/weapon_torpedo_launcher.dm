...
/obj/machinery/power/ship_weapon/torpedo
	...
	var/obj/machinery/door/blast/regular/door

/obj/machinery/power/ship_weapon/torpedo/proc/find_loader()
	if(!loader)
		loader = locate() in get_step(get_turf(src), GLOB.reverse_dir[dir])
		if(loader)
			loader.launcher = src
...
/obj/machinery/power/ship_weapon/torpedo/can_fire()
	if(!loader) loader = locate() in get_step(get_turf(src), GLOB.reverse_dir[dir])
	if(!door) door = locate() in get_step(get_turf(src), src.dir)
	return (loaded && loader && !loader.open && !door.density)
