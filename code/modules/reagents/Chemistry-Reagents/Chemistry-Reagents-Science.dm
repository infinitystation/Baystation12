...
/datum/reagent/rsh4/affect_blood(var/mob/living/carbon/human/M, var/alien, var/removed)
	if(alien != IS_DIONA)
		if(M.gender != MALE)
			...
			to_chat(M, "<span class='notice'>Я чувствую себя... Странно. Мягкое, приятное тепло распространяется по всему моему телу, особенно по груди, почему-то заставляя её тяжалеть... Меня немного пугает ощущение того, как тепло распространяется по моему паху...</span>")
			M.gender = FEMALE
	M.update_icon()
	remove_self(volume)
	return