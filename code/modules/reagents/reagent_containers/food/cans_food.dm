...
/obj/item/reagent_containers/food/snacks/can/two
	name = "canned stew"
...
/obj/item/reagent_containers/food/snacks/can/three
	name = "canned jelly"
...
/obj/item/reagent_containers/food/snacks/can/four
...
/obj/item/reagent_containers/food/snacks/can/five
...
/obj/item/reagent_containers/food/snacks/can/attack_self(mob/user as mob)
	if(!is_open_container() && !diff)
		open(user)

/obj/item/reagent_containers/food/snacks/can/on_update_icon()
	if(!reagents.total_volume)
		icon_state = "[initial(icon_state)]_empty"
		return

/obj/item/reagent_containers/food/snacks/can/attackby(obj/item/W as obj, mob/user as mob)
	if(istype(W,/obj/item/material/knife))
		...
		if(do_after(user, 150, src))
			open(user)
			return
	else if(istype(W,/obj/item/material/canknife) || istype(W, /obj/item/material/knife))
		to_chat(user, "<span class='notice'>You're starting open the [src]!</span>")
		user.visible_message("<span class='warning'>The [user] is trying to open the [src] with [W]!</span>")
		if(do_after(user, 50, src))
			open(user)
			return
	else if(istype(W,/obj/item/material/kitchen/utensil))
...
/obj/item/reagent_containers/food/snacks/can/proc/open(mob/user)
...
/obj/item/reagent_containers/food/snacks/can/attack(mob/M as mob, mob/user as mob, def_zone)
