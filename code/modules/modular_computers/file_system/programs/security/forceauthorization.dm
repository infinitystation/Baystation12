...
/datum/nano_module/forceauthorization/Topic(href, href_list)
	...
	if(href_list["gun"] && ("authorize" in href_list) && href_list["mode"])
		...
		return isnum(do_authorize) && isnum(mode) && G && G.authorize(mode, do_authorize, FindNameFromID(usr)) //inf, was: return isnum(do_authorize) && isnum(mode) && G && G.authorize(mode, do_authorize)
