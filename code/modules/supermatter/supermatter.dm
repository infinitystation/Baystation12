...
/obj/machinery/power/supermatter/proc/announce_warning()
	...
	var/alert_msg = " Öåëîñòü êðèñòàëëà [integrity] ïðîöåíòîâ"
	...
	if(damage > emergency_point)
		...
		alert_msg = null
	if(alert_msg)
		radio.autosay(alert_msg, "Ìîíèòîðèíã Ñóïåðìàòåðèè", "Èíæåíåðíàÿ")
		//Public alerts
		if((damage > emergency_point) && !public_alert)
			radio.autosay("ÂÍÈÀÍÈÅ: ÄÅËÀÌÈÍÀÖÈß ÊÐÈÑÒÀËËÀ ÑÓÏÅÐÌÀÒÅÐÈÈ ÍÅÈÇÁÅÆÍÀ!", "Ìîíèòîðèíã Ñóïåðìàòåðèè")
			public_alert = 1
		else if(safe_warned && public_alert)
			radio.autosay(alert_msg, "Ìîíèòîðèíã Ñóïåðìàòåðèè")
			public_alert = 0
