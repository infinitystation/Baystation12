/mob/living/carbon/human/emote(var/act,var/m_type=1,var/message = null)
	...
	switch(act)
		...
		if ("stare")
			...
			if (param)
				message = "пристально смотрит на [param]."
			else
				message = "пялится."
		...
		if ("cry")
			if(miming)
				...
				if (!muzzled)
					...
					message = "издаЄт слабый звук. [get_visible_gender() == MALE ? "He" : get_visible_gender() == FEMALE ? "She" : "They"] [get_visible_gender() == NEUTER ? "frown" : "frowns"]."
					m_type = 2
		...
		if ("groan")
			if(miming)
				...
				if (!muzzled)
					...
					message = "издаЄт громкий звук."
					m_type = 2

		if ("moan")
			if(miming)
				message = "по своему виду, стонет!"
				m_type = 1
			else
				message = "стонет!"
				m_type = 2
		...
		if ("johnny")
			...
			if (!M)
				...
				if(miming)
					...
					message = "говорит, \"[M], please. He had a family.\" [src.name] takes a drag from a cigarette and blows his name out in smoke."
					m_type = 2
