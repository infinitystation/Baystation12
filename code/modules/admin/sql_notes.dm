/proc/add_note(target_ckey, notetext, timestamp, adminckey, logged = 1, server)
	...
	if(logged)
		...
		send2adminirc("[key_name(usr)] добавил нотес игроку [target_ckey]: [notetext]")
...
/proc/remove_note(note_id)
	...
	send2adminirc("[key_name(usr)] убрал нотес от [adminckey] у игрока [ckey]: [notetext]")
	show_note(ckey)
...
/proc/edit_note(note_id)
	...
	if(query_find_note_edit.NextRow())
		...
		send2adminirc("[key_name(usr)] изменил нотес [adminckey] у игрока [target_ckey] с - [old_note] на - [new_note]")
		show_note(target_ckey)
