	...
	//JOBBAN'S INNARDS
		...
		switch(alert("Temporary Ban?",,"Yes","No", "Cancel"))
			if("Yes")
				...
				var/mins_readable = minutes_to_readable(mins)
				ban_unban_log_save("[usr.client.ckey] has HARD banned [mob_key]. - Reason: [reason] - This will be removed in [mins_readable].")
				add_note(mob_key,"[usr.client.ckey] has hard banned [mob_key]. - Reason: [reason] - This will be removed in [mins_readable].", null, usr.ckey, 0)
