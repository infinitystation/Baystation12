...
/client/proc/mod_list_add(var/list/L, atom/O, original_name, objectvar)
	...
	switch(class)
		...
		if("path")
			var_value = text2path(input("Enter new text:","Text") as text)
...
/client/proc/mod_list(var/list/L, atom/O, original_name, objectvar)
	...
	switch(class) //Spits a runtime error if you try to modify an entry in the contents list. Dunno how to fix it, yet.
		...
		if("path")
			new_var = text2path(input("Enter new path:","Text") as text)
			if(assoc)
				L[assoc_key] = new_var
			else
				L[L.Find(variable)] = new_var
...
/client/proc/modify_variables(var/atom/O, var/param_var_name = null, var/autodetect_class = 0)
	...
	switch(class)
		...
		if("path")
			var/var_new = text2path(input("Enter new path:","Text",O.get_variable_value(variable)) as null|text)
			if(var_new==null) return
			var_value = var_new
