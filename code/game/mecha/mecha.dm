...
/obj/mecha/proc/mechturn(direction)
	set_dir(direction)
	playsound(src,'infinity/sound/mecha/Mech_Rotation.ogg',40,1)
...
/obj/mecha/proc/mechstep(direction)
	...
	if(result)
		for(var/mob/M in src)
			M.set_dir(dir)
		playsound(src,'infinity/sound/mecha/Mech_Step.ogg',100,1)
	return result
...
/obj/mecha/verb/move_inside()
	...
	if(iscarbon(usr))
		...
		if(C.handcuffed)
			...
			return
	if (src.occupant)
		sound_to(usr, sound('infinity/sound/mecha/UI_SCI-FI_Tone_Deep_Wet_15_stereo_error.ogg',channel = 4, volume = 100))
...
/obj/mecha/Topic(href, href_list)
	...
	if(href_list["update_content"])
		...
		return
	if(href_list["close"])
		sound_to(usr, sound('infinity/sound/mecha/UI_SCI-FI_Tone_10_stereo.ogg',channel=4, volume=100))
		return
	if(usr.stat > 0)
		return
	...
	if(href_list["eject"])
		if(usr != src.occupant)	return
		playsound(src, 'infinity/sound/mecha/ROBOTIC_Servo_Large_Dual_Servos_Open_mono.ogg',100,1)
		src.eject()
		return
	...
	if (href_list["change_name"])
		...
		if(newname)
			sound_to(usr, sound('infinity/sound/mecha/UI_SCI-FI_Tone_Deep_Wet_22_stereo_complite.ogg',channel=4, volume=100))
			SetName(newname)
		else
	...
	if(href_list["maint_access"] && maint_access)
		...
		if(user)
			if(state==0)
				state = 1
				sound_to(user, sound('infinity/sound/mecha/UI_SCI-FI_Tone_Deep_Wet_22_stereo_complite.ogg',channel=4, volume=100))
	...
	if(href_list["set_internal_tank_valve"] && state >=1)
		...
		var/mob/user = F.getMob("user")
		if(user)
			sound_to(user, sound('infinity/sound/mecha/UI_SCI-FI_Tone_10_stereo.ogg',channel=4, volume=100))
	...
	if(href_list["dna_lock"])
		...
		if(istype(occupant, /mob/living/carbon/brain))
			...
			return
		if(src.occupant)
			sound_to(usr, sound('infinity/sound/mecha/UI_SCI-FI_Compute_01_Wet_stereo.ogg',channel=4, volume=100))
