...
/obj/machinery/door/blast/shutters/glass/attackby(obj/item/I, mob/user, params)
	if(density)
		if(istype(I, /obj/item/weapon/weldingtool))
			...
			if((stat&BROKEN) && have_glass)
				if(WT.remove_fuel(0,user))
					to_chat(user, "<span class='notice'>You begin slicing [src]'s debris...</span>")
			...
			if(!have_glass)
				...
				if(G.get_amount() >= 2)
					...
					to_chat(user, "<span class='notice'>You start to put the glass into [src]...</span>")
		...
		to_chat(user, "<span class='warning'>It must be closed!</span>")
