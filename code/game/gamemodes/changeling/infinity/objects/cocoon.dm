...
/obj/structure/changeling_cocoon/proc/background()
	...
	Мы станем частью общности - одним из сородичей, что будем работать на её благо и ставить её интересы \
	выше собственных - в том числе и жизни. Мы должны взять под контроль корабль, поглотив его экипаж и превратив \
	тем самым в себе подобных. Пока экипаж не станет слишком малочисленен и слаб, чтобы можно было охотиться без опаски \
	- нам следует проявлять осторожность.<br>\
	Если наш кокон никто не потревожит, то вскоро <i>мы</i> очнёмся и прогрызём себе путь на свободу, включившись в охоту. \
