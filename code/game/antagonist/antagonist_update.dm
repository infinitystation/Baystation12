...
/datum/antagonist/proc/get_indicator(var/datum/mind/recipient, var/datum/mind/other)
	...
	var/image/I = image('infinity/icons/mob/hud.dmi', loc = other.current, icon_state = indicator, layer = ABOVE_HUMAN_LAYER) //INF, was var/image/I = image('icons/mob/hud.dmi', loc = other.current, icon_state = indicator, layer = ABOVE_HUMAN_LAYER)
