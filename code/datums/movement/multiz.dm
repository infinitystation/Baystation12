/datum/movement_handler/mob/multiz/DoMove(var/direction, var/mob/mover, var/is_external)
	...
	if(!destination)
		to_chat(mover, "<span class='notice'>There is nothing of interest in this direction.</span>")
		return MOVEMENT_HANDLED

	var/turf/start = get_turf(mob)
	if(!start.CanZPass(mob, direction))
		to_chat(mover, "<span class='warning'>\The [start] is in the way.</span>")
		return MOVEMENT_HANDLED

	if(!destination.CanZPass(mob, direction))
		to_chat(mover, "<span class='warning'>You bump against \the [destination].</span>")
		return MOVEMENT_HANDLED

	var/area/area = get_area(mob)
	if(direction == UP && area.has_gravity() && !mob.can_overcome_gravity())
		to_chat(mover, "<span class='warning'>Gravity stops you from moving upward.</span>")
		return MOVEMENT_HANDLED
