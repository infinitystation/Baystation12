...
/obj/item/paper/sierra/cryocell
	name = "cryocell notice"
	info = "Алекс сказал, что прошлая криокапсула была взорвана из-за сильного давления по вине какого-то идиота, поэтому нам пришлось списать её. Надо бы потом попросить отдел исследований посмотреть можно ли её чем-то заменить."
