/datum/map/sierra
	...
	meteor_detected_message = "Внимание всему персоналу, обнаружены метеориты прямо по курсу %STATION_NAME%. Всему персоналу настоятельно рекомендуется пройти в центр корабля до выхода из опасной зоны."
...
/datum/map/sierra/level_x_biohazard_announcement(var/bio_level)
	...
	switch(bio_level)
		if(9)
			msg = "неопознанная форма жизни. Рекомендуется изолировать неизвестное. Это максимальный уровень потенциальной угрозы"
		if(8)
		...
		if(4)
			msg = "быстрорастущие растения. Точное определение степени описаности невозможно из-за активных мутаций"
		if(3)
	...
	priority_announcement.Announce("Подтверждено наличие объекта [bio_level] уровня биологической опасности на борту [station_name()] - [msg]. Ответственному экипажу требуется предпринять меры по устранению угрозы.", "Биологическая Угроза")
