...
/datum/computer_file/report/recipient/iaa/salary_deceased/generate_fields()
	...
	add_field(/datum/report_field/date, "Дата")
	add_field(/datum/report_field/time, "Время")
	add_field(/datum/report_field/people/from_manifest, "Данный документ разрешает выплату оставшегося оклада(имя и фамилия погибшего сотрудника NT)", required = 1)
	add_field(/datum/report_field/pencode_text, "Помимо полной выплаты оставшихся личных активов(Актив, Сумма в талерах)", required = 1)
