...
/datum/computer_file/report/recipient/sec/production_ban
	form_name = "NT-SEC-21f"
	title = "Производственный запрет"
	available_on_ntnet = 1

/datum/computer_file/report/recipient/sec/production_ban/generate_fields()
	..()
	add_field(/datum/report_field/text_label/header, "ИКН Сьерра - Охранный департамент")
	add_field(/datum/report_field/text_label/instruction, "Заполняется Капитаном, Агентом Внутренних Дел или главой работника. Документ должен быть подписан и одобрен, чтобы являтся действительным. \
	На всех копиях документа должна стоять печать.")
	add_field(/datum/report_field/people/from_manifest, "Обвиняемый", required = 1)
	add_field(/datum/report_field/people/from_manifest, "Составитель запрета", required = 1)
	add_field(/datum/report_field/date, "Дата вступления в силу")
	add_field(/datum/report_field/time, "Время вступления в силу")
	add_field(/datum/report_field/pencode_text, "Описание запрета", required = 1)
	add_field(/datum/report_field/signature, "Подпись составителя", required = 1)
