...
/datum/codex_entry/docking_port
	...
	объктах - или хотя бы в порту. Подобные системы часто занимают прилично места, не говоря уже о \
	...
	инженерной команде."
	mechanics_text = "Док может сломаться, если корабль начнет движение с выдвинутым доком. \
	Обычно, другие суда могут свободно подключаться к ним, если отсутствуют повреждения и док не \
	помечен как \"reinforced\" (укрепленный).<br>\
	Для ремонта необходим продвинутый навык конструирования. Этапы: гаечный ключ, пустая рука (2), \
