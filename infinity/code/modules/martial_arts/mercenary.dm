...
//Powerful, but hard to aquire martial art
...
/datum/martial_art/cqc/handle_grab(var/mob/living/carbon/human/owner, var/mob/living/carbon/human/victim)
	if(owner != victim)
		owner.visible_message(SPAN_DANGER("[owner] attempted to grab \the [victim]!"))

	var/obj/item/grab/G
	G = new owner.current_grab_type(owner, victim)
	G.upgrade(TRUE)
