...
/obj/effect/multitile_entrance
	...
	var/obj/vehicle_infinity/multitile/root/master
	invisibility = 101
	anchored = TRUE
...
/obj/vehicle_infinity/multitile
...
/obj/vehicle_infinity/multitile/hitbox
	name = "Hitbox"
	desc = "Generic multitile vehicle hitbox"

	var/obj/vehicle_infinity/multitile/root/root
	invisibility = 101
/obj/vehicle_infinity/multitile/root
	name = "Root"
	...
	var/hitbox_type = /obj/vehicle_infinity/multitile/hitbox

//How to get out, via verb
/obj/vehicle_infinity/multitile/root/verb/exit_multitile()
	set category = "Vehicle"	//changed verb category to new one, because Object category is bad.
	set name = "M Exit Vehicle"
	set src = usr.loc
...
/obj/vehicle_infinity/multitile/root/proc/handle_player_exit(var/mob/M)
	return

/obj/vehicle_infinity/multitile/root/proc/handle_player_entrance(var/mob/M)
	return

/obj/vehicle_infinity/multitile/root/proc/handle_harm_attack(var/mob/M)
	return

//Vebrs for rotations, set up a macro and get turnin
/obj/vehicle_infinity/multitile/root/verb/clockwise_rotate_multitile()
	set category = "Vehicle"	//changed verb category to new one, because Object category is bad.
	set name = "M Rotate Vehicle Clockwise"
	set src = usr.loc

	var/mob/M = usr
	try_rotate(-90, M)

/obj/vehicle_infinity/multitile/root/verb/counterclockwise_rotate_multitile()
	set category = "Vehicle"	//changed verb category to new one, because Object category is bad.
	set name = "M Rotate Vehicle Counterclockwise"
	set src = usr.loc
...
/obj/vehicle_infinity/multitile/root/proc/try_rotate(var/deg, var/mob/user, var/force = 0)
	save_locs()
	rotate_coords(deg)
...
/obj/vehicle_infinity/multitile/root/relaymove(mob/user, direction)
...
/obj/vehicle_infinity/multitile/root/New()
...
/obj/vehicle_infinity/multitile/root/proc/load_hitboxes(var/datum/coords/dimensions, var/datum/coords/root_pos)
	return

/obj/vehicle_infinity/multitile/root/proc/load_entrance_marker(var/datum/coords/rel_pos)
	return
//Saves where everything is so we can revert
/obj/vehicle_infinity/multitile/root/proc/save_locs()
...
/obj/vehicle_infinity/multitile/root/proc/revert_locs()
...
/obj/vehicle_infinity/multitile/root/proc/move_root(var/direction)
...
/obj/vehicle_infinity/multitile/root/proc/try_move(var/list/objs, var/direction, var/is_rotation = 0)
	...
	for(var/datum/coords/C in objs) //objs is an associative list like linked_objs
		...
		if(is_rotation) //Special case for rotations where two hitboxes need to swap locations
			...
			var/obj/vehicle_infinity/multitile/M
			for(var/i in T) //Get the one to swap with
				if(istype(i, /obj/vehicle_infinity/multitile))
					M = i
					break
			if(istype(M))
...
/obj/vehicle_infinity/multitile/root/proc/rotate_coords(var/deg)
